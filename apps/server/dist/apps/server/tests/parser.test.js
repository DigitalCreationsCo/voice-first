...
import { createParser, parseChunk } from '../parser';
describe('StreamParser', () => {
    it('parses static fields and streams text correctly', () => {
        ...
        parser = p1;
        const meta = u1.find(u => u.type === 'meta');
        expect(meta).toBeDefined();
        expect(meta.data.rating).toBe(5);
        expect(meta.data.difficulty).toBe(2);
        const stream = u1.find(u => u.type === 'stream');
    ...
    it('parses incrementally streamed text chunks', () => {
        const config = { keys: ['text'], streamKeys: ['text'] };
        let parser = createParser(config);
        ...
        const streamDeltas = allUpdates.filter(u => u.type === 'stream');
    ...
    it('ignores malformed or incomplete segments until finalized', () => {
        const config = { keys: ['rating', 'text'], streamKeys: ['text'] };
        let parser = createParser(config);
        ...
        parser = p2;
        const meta = u2.find(u => u.type === 'meta');
        expect(meta.data.rating).toBe(5);
        const { parser: p3, updates: u3 } = parseChunk(parser, 'text: Every');
        parser = p3;
        const stream = u3.find(u => u.type === "stream");
        expect(stream.delta).toBe('Every');
    });
    it('trims semicolons and parses numeric fields correctly', () => {
        const config = { keys: ['rating', 'difficulty', 'text'], streamKeys: ['text'], terminator: ';', delimiter: ':' };
        let parser = createParser(config);
    ...
    it('parses keys split across chunks', () => {
        const config = { keys: ['rating', 'difficulty', 'text'], streamKeys: ['text'] };
        let parser = createParser(config);
        ...
        const meta = allUpdates.find(u => u.type === 'meta');
    ...
    it('parses jsonKeys when optionalKeys is omitted and when present', () => {
        ...
        let allUpdatesOptional = [];
        for (const c of chunks) {
            const { parser: p, updates } = parseChunk(parserOptional, c);
            parserOptional = p;
            allUpdatesOptional.push(...updates);
        }
        const metaOptional = allUpdatesOptional.find(u => u.type === 'meta');
        const streamOptional = allUpdatesOptional.find(u => u.type === 'stream');
        ...
        let allUpdatesNoOptional = [];
        for (const c of chunks) {
            const { parser: p, updates } = parseChunk(parserNoOptional, c);
            parserNoOptional = p;
            allUpdatesNoOptional.push(...updates);
        }
        const metaNoOptional = allUpdatesNoOptional.find(u => u.type === 'meta');
        const streamNoOptional = allUpdatesNoOptional.find(u => u.type === 'stream');
    ...
    it('handles optional key absent and emits skip when next key appears first', () => {
        const config = { keys: ['rating', 'difficulty', 'text'], streamKeys: ['text'], optionalKeys: ['difficulty'] };
        let parser = createParser(config);
    ...
    it('handles all optional key absent and emits skip update', () => {
        const config = { keys: ['rating', 'difficulty', 'text'], streamKeys: ['text'], optionalKeys: ['rating', 'difficulty'] };
        let parser = createParser(config);
        const chunk = 'text: Test message;';
        const { parser: p, updates } = parseChunk(parser, chunk);
        parser = p;
        const skip1 = updates.find((u) => u.key === 'rating');
        expect(skip1).toBeDefined();
        const skip2 = updates.find((u) => u.key === 'difficulty');
    ...
    it('handles multiple stream chunks and accumulates parsed text', () => {
        ...
        const streamText = allUpdates.filter(u => u.type === 'stream').map(u => u.delta).join('');
...
`];
        ...
        const streamText2 = allUpdates2.filter(u => u.type === 'stream').map(u => u.delta).join('');

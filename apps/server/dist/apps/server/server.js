...
// To automatically transpile TypeScript custom paths (like '@/shared/...') to relative paths during build,
// you can use a tool such as `tsc-alias` or `typescript-transform-paths`.
// 1. Install tsc-alias as a dev dependency:
//    npm install --save-dev tsc-alias
// 2. Run tsc to compile your project, then run tsc-alias to rewrite the paths in the output files:
//    npx tsc && npx tsc-alias
// This way, you keep source code path imports, and in dist the paths will be rewritten as relative imports.
// For example, after build, this import—
import AudioDebugger, { AudioFormat } from '@/shared/audio/audio-helpers';
// —would become:
//// import AudioDebugger, { AudioFormat } from '../../../shared/audio/audio-helpers.js';
import { createParser, parseChunk } from './parser.js';
config({
    path: ".env.local",
});
                ...
                if (candidate && candidate.content?.parts) {
                    for (const part of candidate.content.parts) {
                        if (part.inlineData && part.inlineData.data) {
                            const isValidBase64 = AudioDebugger.validate(part.inlineData.data, AudioFormat.BASE64_STRING);
                            if (!isValidBase64) {
